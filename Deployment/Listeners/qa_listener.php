...
try {
    ...
    $server = new RabbitMQServer(__DIR__ . '/../Deployment.ini', 'Deployment-QA');
    ...
    $server->consume(function ($message) use ($db, $server) {
        ...
        if (!isset($data['action'])) {
            ...
            switch ($action) {
                // On demand bundle request from QA
                case 'get_bundle':
                    ...
                    if ($bundle) {
                        ...
                        $server->publishMessage([
                            'action' => 'new_bundle', //This action triggers installation on the QA side
                ...
                case 'update_status':
                    ...
                    if ($stmt->execute()) {
                        ...
                        if (preg_match('/^([A-Za-z0-9]+)_v[0-9]+\.[0-9]+\.[0-9]+\.zip$/', $bundle_name, $matches)) {
                            ...
                            // Optional: automatically rollback to latest 'passed' bundle of same category after marking a bundle as 'failed'
