...
foreach ($portfolio as $coin) {
    foreach ($crypto as $crypto_data) {
        if (strtolower($coin['coin_symbol']) === strtolower($crypto_data['symbol'])) {
            ...
            $risk_factor = abs(floatval($crypto_data['change_percent']));
            $weighted_risk_sum += $risk_factor * $total_value;
            $coin['change_percent'] = floatval($crypto_data['change_percent']);
            break;
        }
    }
}
...
usort($crypto, function ($a, $b) {
    return abs(floatval($b['change_percent'])) <=> abs(floatval($a['change_percent']));
});


$safer_recommendations = array_slice(array_filter($crypto, function ($c) {
    return abs(floatval($c['change_percent'])) < 1;
}), 0, 5);


$riskier_recommendations = array_slice(array_filter($crypto, function ($c) {
    return abs(floatval($c['change_percent'])) >= 3;
}), 0, 5);
    ...
    <div class="recommendations">
        ...
        <ul id="coin-recommendations" class="recommendation-list">
            <?php foreach ($safer_recommendations as $recommendation): ?>
                <li><?= htmlspecialchars($recommendation['name']) ?> (<?= htmlspecialchars($recommendation['symbol']) ?>) (Change: <?= htmlspecialchars(number_format($recommendation['change_percent'], 2)) ?>%)</li>
            <?php endforeach; ?>
        </ul>
    </div>
</div>
        ...
        function displayRecommendations(recommendations) {
            ...
            if (recommendations.length > 0) {
                recommendations.forEach(coin => {
                    ...
                    li.textContent = `${coin.name} (${coin.symbol}) (Change: ${parseFloat(coin.change_percent).toFixed(2)}%)`;
