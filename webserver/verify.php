...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['new_phone_number']) && $changingPhoneNumber) {
    	...
    	$userPhoneNumber = $newPhoneNumber; // Update displayed number
    	$changingPhoneNumber = false; // Go back to verification display
	} elseif (isset($_POST['phone_number']) && !$changingPhoneNumber && !isset($_POST['verification_code'])) {
    	// Step 1: User submitted phone number (either fetched or new), send verification code
    	...
    	// Step 2: User submitted verification code
    	...
    	try {
        	...
        	if ($verificationCheck->status === 'approved') {
            	// Mark the user as verified in the session (you might want to update the database here as well)
            	$_SESSION['is_verified'] = true;
            	header('Location: crypto-side/home.php'); // Redirect to home.php
            	exit();
        	} else {
...
<style>
	...
	.logout-link {
    	display: block;
    	margin-top: 25px;
    	color: orange; /* Using your original logout link color */
    	text-decoration: none;
    	font-weight: bold; /* Using your original logout link font weight */
    	font-size: 0.9em;
	}
	.logout-link:hover {
