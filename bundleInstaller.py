...
def process_bundle_message(ch, method, properties, body):
    try:
        ...
        if action == 'new_bundle' or action == 'rollback_bundle':
...
def install_bundle(zip_filename):
    ...
    try:
        ...
        scp_command = f"sshpass -p '!Bundle123' scp -o StrictHostKeyChecking=no {ssh_user}@{ssh_host}:{remote_path}/{zip_filename} {local_temp_path}/"
        ...
        unzipped_files_path = local_temp_path
        for filename in os.listdir(unzipped_files_path):
            if filename.endswith(".php") or filename.endswith(".html") or filename.endswith(".css") or filename.endswith(".js"):
                file_path = os.path.join(unzipped_files_path, filename)
                if os.path.isfile(file_path) and filename != zip_filename:
                    destination_path = f"/var/www/html/{filename}"
                    shutil.copy(file_path, destination_path)
                    logging.info(f"Replaced {destination_path}")
...
def main():
    ...
    while True:
        try:
            logging.info(f"Attempting to connect to RabbitMQ: {broker_host}:{broker_port} on vhost '{vhost}' as user '{user}'")
            ...
            logging.info(f"Successfully connected to RabbitMQ. Waiting for messages on queue '{queue}'.")
            channel.start_consuming()  # This will block and wait for messages
            break  # If connection and consuming starts, exit the loop
